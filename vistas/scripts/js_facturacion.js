      ...
      if (e.status == true) {    
        ...
        $.each(e.data.metodo_pago, function (index, val2) { 
           if (index == 0) {
            ...
            if (val2.comprobante == null || val2.comprobante == '') {   } else {          
              if (UrlExists(`../assets/modulo/facturacion/ticket/${val2.comprobante}`) == 200) {
                // filePondInstances[0].addFile(`../assets/modulo/facturacion/ticket/${val2.comprobante}`, { index: 0 });
                file_pond_mp_comprobante = FilePond.create(document.querySelector('#f_mp_comprobante_1'), FilePond_Facturacion_LabelsES ).addFile(`../assets/modulo/facturacion/ticket/${val2.comprobante}`, { index: 0 });
              } else {
                toastr_error('Erro de carga!!',`Hubo un error en la carga de tu comprobante de pago. <br> ${val2.metodo_pago}: ${val2.codigo_voucher}`);
              }          
            }  
           } else {
